...
--how many percet agents have the highest and lowest CSAT scores?
WITH total AS (
...
select Supervisor,round(avg([CSAT Score]),2) as Avg_csat_score,COUNT(Agent_name) as No_of_agents from customer
group by Supervisor
order by avg_csat_score desc;

--What are the most common issues reported by customers?
select top 10 category,[Sub-category] as issues,COUNT(category) as Frequency from customer
group by [sub-category],category
order by Frequency desc

--How does customer satisfaction correlate with different product categories?
select category,round(avg([CSAT Score]),2) from customer
group by category

--What is the average order value associated with different customer complaints?
select category,AVG(cast(Item_price as int)) as average_order_value from customer
where Item_price is not null
group by category
order by average_order_value desc

--How do issue response times differ by customer city?
-- Using a subquery to calculate hours and minutes
SELECT Customer_City,hr,minu
FROM (	
		SELECT Customer_City,ROUND(CAST(Response_Time AS INT) / 60, 0) AS hr,
        CAST(CAST(Response_Time AS INT) % 60 AS INT) AS minu
		FROM customer
		WHERE Customer_City IS NOT NULL) AS Subquery
WHERE hr > 0 OR minu > 0
ORDER BY hr DESC, minu DESC;


--What trends can be identified in customer remarks over time?
select top 10 [Customer Remarks],count([Customer Remarks]) as frequency from customer 
